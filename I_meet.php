...
<?php
$menuright=menuright(84);//取得菜单权限
$tit='会议管理 <span class="c-gray en">&gt;</span> 会议资料-- '.$_SESSION['empid'];
...
<select class="select-box" style="width:80px;height:31px;text-align:center;" name="zt"><option value="">--状态--</option><option value="0">预定中</option><option value="1">审核通过</option><option value="2">预定失败</option></select>
...
$lnk='<span class="l"><a href="I_meet.php"  class="btn radius"><img border=0 src=im/zhuy.png> 会议管理</a> </span> ';
$lnk.='<span class="r">';
if($menuright==3){
    $lnk.='<div onMouseMove="show()" onMouseOut="hide()" class="btn radius" style="background:#ffffff;" >批量审核<br>
    <div id="menu1" style="position:absolute;display:none;width:130;text-align:left;line-height:25px;background-color:#ffffff;border:1px solid #68AF7E">
     <a href="javascript:s_all(1)"><font color="#6495ed">会议审核通过</a><br>
     <a href="javascript:s_all(2)"><font color="#6495ed">会议审核不过</a>
    </div></div>';
}
if($menuright==2){
    $lnk.='<a href="javascript:;" onclick="Add(\'会议管理--新增会议\',\''.$xiam.'Add.php\',\'750\',\'600\')" class="btn radius"><img border=0 src=im/add.png> 新增会议</a>';
...
$cha='';
if($menuright==2){
$lie=',序,会议类型,编号,会议议题,开始时间,结束时间,预约时间,会议室,参会人数,参会人员,会议纪要,预约人,状态,修改';
$wid=',3%,10%,6%,15%,8%,8%,8%,6%,5%,15%,4%,4%,4%,4%';
}
else{
    $lie=',序,会议类型,编号,会议议题,开始时间,结束时间,预约时间,会议室,参会人数,参会人员,会议纪要,预约人,状态';
    $wid=',3%,12%,6%,15%,8%,8%,8%,6%,5%,15%,4%,5%,5%';
}
