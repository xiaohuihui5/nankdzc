...
from sys_meet a,sys_meetlx b,sys_fangj f,sys_user u where a.lx=b.id and a.fangj=f.id and a.fqr=u.empid ".$TJ." order by ".$px;
        ...
        $_SESSION['mac'].="#".",序,会议类型,编号,会议议题,开始时间,结束时间,预约时间,会议室,参会人数,参会人员,会议纪要,预约人,状态,修改";
        }
        else{
            $_SESSION['mac']="select 0,0,b.fenlmc,a.bianh,a.yiti,convert(varchar(20),a.ksrq,120),
convert(varchar(20),a.jsrq,120),convert(varchar(20),a.ydrq,120),f.fangjh,a.rens,a.reny,'<a href=javascript:xq('+cast(a.id as varchar(10))+')><img border=0  src=im/select.gif></a>',u.name,case a.yn when 0 then '<a href=javascript:sh('+cast(a.id as varchar(10))+')><font color=red>预定中...' when 1 then '<a href=javascript:sh('+cast(a.id as varchar(10))+')><font color=blue>审核通过' when 2 then '<a href=javascript:sh('+cast(a.id as varchar(10))+')><font color=grey>预定失败' end from sys_meet a,sys_meetlx b,sys_fangj f,sys_user u where a.lx=b.id and a.fangj=f.id and a.fqr=u.empid ".$TJ." order by ".$px;
            //echo $_SESSION['mac'];
            ...
            $_SESSION['mac'].="#".",序,会议类型,编号,会议议题,开始时间,结束时间,预约时间,会议室,参会人数,参会人员,会议纪要,预约人,状态";
        }
        $_SESSION['mac'].="#会议预定";
        $_SESSION['mac'].="#";
        $_SESSION['mac'].="#";
...
<script language=javascript>
    function xq(id)
    {
        ...
        layer_show3("会议预定--会议纪要","<?php echo $xiam;?>Xiangq.php?eid="+id,"750","600","parent");
    }
    function sh(id)
    {
        window.Frm.scroll.value=document.body.scrollTop;
