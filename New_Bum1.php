...
{
	$query="update sys_bum set yn=yn^1 where id=".$_POST['delrow'];
	require('./inc/xexec.php');
}
if(isset($_POST['fenlmc']) || $_POST['fenlmc']!="")
{
	$query="insert into sys_bum(bianh,bummc,yn) values('".$_POST['bianh']."','".$_POST['fenlmc']."',1)";
	//echo $query;
	require("./inc/xexec.php");
	if($res)
	{
...
if(isset($_POST['edtrow']) and $_POST['edtrow']!=0)
{
	...
	{
	$query="update sys_bum set bianh='".$_POST['bianh_']."',bummc='".$_POST['fenlmc_']."' where id=".$_POST['edtrow'];
	require("./inc/xexec.php");
	if($res)
	{
		echo "<script language=javascript>parent.layer.msg('操作成功！',{icon:1,time:1500});</script>";//提示成功退出
...
<?php 
$query="select 0,a.id,a.bianh,a.bummc,case a.yn when 1 then '启用' else '<font color=gray>停用' end from sys_bum a  order by a.bianh";
