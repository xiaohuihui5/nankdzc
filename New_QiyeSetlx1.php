...
{
    $query="update sys_qiyefenl set yn=yn^1 where id=".$_POST['delrow'];
    require('./inc/xexec.php');
}
if(isset($_POST['fenlmc']))
{
    $query="insert into sys_qiyefenl(bianh,fenlmc,yn) values('".$_POST['bianh']."','".$_POST['fenlmc']."',1)";
    //echo $query;die;
    require("./inc/xexec.php");
}
$ed_row=0;
if(isset($_POST['edtrow']) and $_POST['edtrow']!=0)
{
    if(isset($_POST['bh_']))
    {
        $query="update sys_qiyefenl set bianh='".$_POST['bh_']."',fenlmc='".$_POST['fenlmc_']."' where id=".$_POST['edtrow'];
        require("./inc/xexec.php");
        $ed_row=0;
    }
...
<form action="" method=post name="Frm">
    ...
    <table border="0" class="tableborder3">
        <?php
        $query="select 0,a.id,case len(a.bianh) when 0 then null else a.bianh end,a.fenlmc,case a.yn when 1 then '在用' else '<font color=red>禁用</font>' end from sys_qiyefenl a  order by a.bianh,a.fenlmc";
