...
{
	var Win = window.open(url,'','width='+w+',height='+h+',left='+(window.screen.availWidth-w)/2+',top='+(window.screen.availHeight-h)/2+',resizable=1,scrollbars=no,menubar=no,status=no');
}
...
function check(e,lx)
{
	...
	{
		//layer.confirm('本输入框只能输入数字和点号!',{btn:["确定"],shade:0.2,});
		alert("本输入框只能输入数字和点号!");
		window.event.keyCode = 0 ;
	}
}
...
function layer_show2(title,url,w,h,par){
		layer.open({
			...
			shade:0.2,//遮罩
			moveOut : false,
			offset:[($(document).height()-h)/2,($(document).width()-w)/2],  
			title: title,
			content: url,
			end:function () {}
		});
...
function layer_show3(title,url,w,h,par){
		parent.layer.open({
			...
			offset:[($(parent.document).height()-h)/2,($(parent.document).width()-w)/2],  
			title: title,
			content: url,
			end:function () {}
		});
...
function SelCp(lx)//弹出产品选取
{
	if(lx==1)
		layer_show2("产品选取","Cs_SelectCp.php","500","500","parent"); //最后一个参数是给一个标识符 
	else
		layer_show2("产品选取","I_Sell1Mx_Cp.php","500","500","parent"); //最后一个参数是给一个标识符 
