...
//$yul预留参数，目前没用
	...
	for($i=1;$i<count($l);$i++)
	{
		if(isset($p[$i]) && $p[$i]!='')   
		{
			...
			$yn=false;
  			$l_m.='<th align="center" width="'.$w[$i].'" height="20"><a href=javascript:s('.$p[$i].')><font color=white><b>'.$l[$i].'</b><span id="m'.$p[$i].'">'.$fir.'</span></th>';
		}
		else
  			$l_m.='<th align="center" width="'.$w[$i].'" height="20"><b>'.$l[$i].'</b></th>';
	}
$l_m.='</tr>';
echo <<<LEND
<table border="0" width="100%" height="100%" cellSpacing="1" cellPadding="0">
<tr><td><nav class="breadcrumb">{$tit} <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav></td></tr>
<tr><td>{$lur}</td></tr>
<tr><td><div class="page-container"><div class="cl pd-5 bg-1 bk-gray mt-20">{$lnk}</div></div> </td></tr>
...
<tr>
<td>
<table class="table table-border table-bordered table-hover table-bg table-sort">{$l_m}</table>
</td>
<td width="16">&nbsp;</td>
</tr>
</table>
