...
sqlsrv_free_stmt($result);
//常用
$query="select id,mc,dw,gg from sys_cp where id in(select cpid from weix_changy where useid=".$kehid.") and yn=1 and id in(".$cp_id.") order by bh";
$result=sqlsrv_query($conn,$query);
while($line=sqlsrv_fetch_array($result))
{
...
<input id="prod_num" name="prod_num" type="hidden" value=""></input>
        ...
        <div class="bottom_bod">
            <p class="bottom_img"><span class="hint" id="cart_goods_num">3</span></p>
            <p class="bottom_time"><span class="bottom_time_sapn" id="cart_total_price">￥134.50</span></p>
        </div>
