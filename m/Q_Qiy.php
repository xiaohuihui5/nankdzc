...
<div class="tab month_sel"><span class="jlp2"><p>可左右滑动下表,点列名排序,点企业所在行查看详情</p></span><!--<a class="jlp3" href="Jy_Main.php"></a>--></div>
...
<tr style="border:0px;margin:0;padding:0;"><td style="border:0px;margin:0;padding:0;"><h2>企业类型:
...
<?php
	$lie=array('0','1','序','企业名称','联系人','联系电话','股权情况','位置');
...
if(isset($_POST['cxtj']) && $_POST['cxtj']!="")
	$TJ.=" and q.shortname like '%".$_POST['cxtj']."%' ";
if(isset($_POST['qylx']) && $_POST['qylx']!="")
	$TJ.=" and q.lx=".$_POST['qylx'];
if($qyid!="")
	$TJ.=" and q.id in(".$qyid.") ";
$query="select 0,q.id,q.shortname,q.fuzy,q.phone,'',q.address,'',a.fenlmc,q.usercode from sys_qiye q,sys_qiyefenl a where q.lx=a.id ".$TJ." order by ".$paix;
echo $query;
$result=sqlsrv_query($conn,$query);
$row=0;$yudl=0;$kc=0;
while($line=sqlsrv_fetch_array($result))
...
$('#biao tr').click(function(){
	location.href="Q_QiyMx.php?id="+$(this).attr("id");
});
</script>
