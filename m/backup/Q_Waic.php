...
<div class="notice_front">
	<div id="news">
		<div class="i_c">
			...
			<div class="i_cont">
<?php
$TJ=" and dh.lx=1 and dh.dhrq between '".$_SESSION['DT1']."' and '".$_SESSION['DT2']."' ";
...
$macm="select 0,right(convert(varchar(10),dh.dhrq,120),5) as rq,unit.shortname,sum(sj.shul),sum(sj.shisl),cast(sum(sj.shisje)/sum(sj.shisl) as decimal(10,2)) as dj,sum(sj.shisje) from sys_jhdh dh,sys_jhsj sj,sys_unit unit where dh.id=sj.dhid and dh.unit=unit.id ".$TJ." group by dh.dhrq,unit.shortname order by ".$_POST['lie'].$_POST['sx'];
$macm.="#"."2,0,0,1,1,0,1";//前面2表示隔2个开始合计,后面如果是1表示要合计
$macm.="#".",,,,,,";//百分比
$macm.="#".",center,left,right,right,right,right";
$macm.="#".",日期,供应商,数量,重量,均价,金额";
$macm.="#".",2,3,4,5,6,7";//排序
$macm.="#";
